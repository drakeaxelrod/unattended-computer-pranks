#!/bin/bash
################################################################################
# OSX Advanced Persistent Threat Simulation
# Security Awareness Training Tool - macOS Version
#
...
function spawn_module() {
    ...
    osascript -e "tell application \"Terminal\"
        do script \"cd '$TEMP_DIR' && bash '$SCRIPT_PATH' '$module_name' '$gen' '$INTENSITY' '$MODE'; exit\"
        set custom title of window 1 to \"${module_name}_NODE_$RANDOM\"
    end tell" &>/dev/null &
...
function run_infection() {
    ...
    local start_time=$(date +%s)
    local files=("kernel32.dll" "ntdll.dll" "user32.dll" "advapi32.dll" "msvcrt.dll" "shell32.dll")
    ...
    while ! should_exit_module "$LIFETIME_MEDIUM" "$start_time"; do
        ...
        echo -e "\033[33m[$(date +%T)] ${action[$((RANDOM % ${#action[@]}))]} $file... ${status[$((RANDOM % ${#status[@]}))]} \033[0m"
        sleep $(random_between 1 3)

        if ((RANDOM % 10 == 0)); then
            play_sound "alert"
            echo -e "\033[31m[!] Process hollowing detected in PID $((1000 + RANDOM % 9000))\033[0m"
        fi
    done
...
function run_glitch() {
    ...
    while ! should_exit_module "$LIFETIME_SHORT" "$start_time"; do
        ...
        for ((i=0; i<20; i++)); do
            local random_text=$(LC_ALL=C tr -dc 'A-Za-z0-9!@#$%^&*' < /dev/urandom | head -c $((10 + RANDOM % 50)))
...
function run_keylogger() {
    ...
    local start_time=$(date +%s)
    local usernames=("admin" "root" "user" "administrator" "john.doe" "jane.smith")
    local domains=("CORP" "DOMAIN" "WORKGROUP" "ENTERPRISE")
    ...
    while ! should_exit_module "$LIFETIME_MEDIUM" "$start_time"; do
        ...
        local pass=$(LC_ALL=C tr -dc 'A-Za-z0-9!@#$%^&*' < /dev/urandom | head -c 12)

        echo "[$(date +%T)] Captured: $domain\\$user : $pass"
...
function run_lateral_movement() {
    ...
    local start_time=$(date +%s)
    local hostnames=("DC01" "WEB01" "DB01" "APP01" "FILE01" "MAIL01" "SQL01")
    ...
    while ! should_exit_module "$LIFETIME_MEDIUM" "$start_time"; do
        ...
        local methods=("PsExec" "WMI" "SMB" "RDP" "SSH" "WinRM")
...
function run_ransomware() {
    ...
    local extensions=(".doc" ".xls" ".pdf" ".jpg" ".png" ".sql" ".db" ".zip")
    local paths=("Documents" "Desktop" "Downloads" "Pictures" "Videos" "Music")
    local encrypted=0
    ...
    while ! should_stop; do
        ...
        local filename=$(LC_ALL=C tr -dc 'a-z' < /dev/urandom | head -c 8)

        echo "[ENCRYPTING] /Users/$USER/$path/$filename$ext"
...
function run_data_exfil() {
    ...
    local filetypes=("credentials.db" "emails.pst" "financial_data.xlsx" "customer_info.csv" "source_code.zip")
...
function run_chat() {
    ...
    local messages=(
        ...
        "AV disabled on critical servers."
...
function run_browser_cascade() {
    ...
    while ! should_stop && [[ $count -lt 10 ]]; do
        ...
        open -na "Safari" --args "$url" &>/dev/null &
...
function run_master() {
    ...
    # Kill all spawned terminals (be careful - this targets Terminal.app)
    osascript -e 'tell application "Terminal" to close (every window whose custom title contains "_NODE_")' 2>/dev/null
...
function show_consent() {
    ...
    echo -e "\033[90m  |  - Active Directory domain controllers                              |\033[0m"
